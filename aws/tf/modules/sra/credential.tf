  ...
  policy = jsonencode({
            ...
            "Statement": [
              ...
              },
              {
                ...
                "Condition": {
                  "StringEquals": {
                    "ec2:vpc": "arn:aws:ec2:${var.region}:${data.aws_caller_identity.current.account_id}:vpc/${module.vpc.vpc_id}"
                  }
                }
              },
              {
              ...
              },
              {
                "Effect": "Allow",
                "Action": "iam:PassRole",
                "Resource": "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.resource_prefix}-s3-instance-profile"
              }
            ]
          }
  )
