...
module "vpc" {
  ...
  private_subnet_names = [for az in data.aws_availability_zones.available[0].names : format("%s-private-%s", var.resource_prefix, az)]
  private_subnets      = var.private_subnets_cidr

  intra_subnet_names = [for az in data.aws_availability_zones.available[0].names : format("%s-privatelink-%s", var.resource_prefix, az)]
...
resource "aws_security_group" "sg" {
  ...
  dynamic "egress" {
    for_each = var.network_configuration != "custom" && !local.is_serverless ? [1] : []
    content {
