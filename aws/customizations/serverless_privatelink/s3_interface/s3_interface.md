...
2. Copy `main.tf`, `variables.tf`, and `outputs.tf` from `customizations/serverless_privatelink/s3_interface` into the new directory. Do **not** copy `versions.tf` as-is — it declares a `provider "aws"` block, and a child module that receives its provider from the root (and uses `count`) must not contain one. Instead, create a `versions.tf` in the module directory containing only the provider requirement, so Terraform can resolve the `aws` provider you pass in (this silences the "Reference to undefined provider" warning):
```hcl
terraform {
  required_providers {
    aws = {
...
```
3. Add the following variable to `aws/tf/variables.tf`:
```hcl
...
```hcl
  private_endpoint_rules = concat(
    ...
    var.enable_s3_privatelink ? [{
      key              = "s3" # static for_each key: endpoint_service is computed and unknown at plan time
