...
Complete reference for all Azure SRA configuration options.
...
- [Service Principal Deployment](#service-principal-deployment)
...
| `location` | string | Azure region (e.g., `"westus2"`, `"eastus"`) |
...
Configure Compliance Security Profile (CSP), Enhanced Security Monitoring (ESM), and Enhanced Security Compliance (ESC) features.

**Available Fields:**

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `compliance_security_profile_enabled` | bool | `null` | Enable Compliance Security Profile |
| `compliance_security_profile_standards` | list(string) | `[]` | Compliance standards (e.g., `["HIPAA"]`) |
| `enhanced_security_monitoring_enabled` | bool | `null` | Enable Enhanced Security Monitoring |
...
**Why disable CMK?**
- Simplified key management requirements
- Organizational policy doesn't require CMK
...
Deploy SRA using a service principal instead of a user identity.

<Admonition type="tip" title="Automated Deployments">

Service principal deployment is recommended for CI/CD pipelines and automated deployments where user interactive login is not feasible.
...
Alternatively, see [this](https://learn.microsoft.com/en-us/azure/databricks/admin/admin-concepts#azure-admins) doc on creating a custom role
...
SRA automatically grants the deploying principal (user or service principal) workspace admin permissions. No additional configuration needed.

**Behavior:**
- User identity: Granted admin on first workspace launch
- Service principal: Automatically granted admin during deployment

### Troubleshooting

See [Troubleshooting - Service Principal Issues](./troubleshooting.mdx#service-principal-issues) for common problems and solutions.
